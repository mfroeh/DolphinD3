...
namespace Dolphin.DevTools
{
    public class PlayerUtilityService : IPlayerUtilityService
    {
        ...
        public async Task TestPrimaryResource()
        {
            ...
            var matches = new Dictionary<PlayerResource, float>();
            foreach (var x in System.Enum.GetValues(typeof(PlayerResource)))
            {
                if ((PlayerResource)x == PlayerResource.None || ((PlayerResource)x).ToString().StartsWith("Secondary"))
                    continue;

                var bitmap = await resourceService.Load((PlayerResource)x);
                var match = await ImageHelper.Compare(bitmap, primaryBitmap, 0);
                matches.Add((PlayerResource)x, match);
            }
            var bestMatch = matches.OrderBy(x => x.Value).Last();
        ...
        public async Task TestSecondaryResource()
        {
            ...
            foreach (var x in System.Enum.GetValues(typeof(PlayerResource)))
            {
                if ((PlayerResource)x == PlayerResource.None || ((PlayerResource)x).ToString().StartsWith("Primary"))
                    continue;
